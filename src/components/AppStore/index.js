...
class AppStore extends Component {
  ...
  getFilteredTab = () => {
    ...
    const filteredTab = appsList.filter(
      eachTab =>
        eachTab.category === activeTabId &&
        eachTab.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return filteredTab
  }
  ...
  render() {
    const {activeTabId, searchInput} = this.state
    const filteredAppTab = this.getFilteredTab()
    ...
    return (
      <div className="app-container">
        ...
        <ul className="appitem-card">
          {filteredAppTab.map(eachApp => (
